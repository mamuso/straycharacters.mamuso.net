...
Responsive image `sizes` follow each photograph's share of its gallery row, including gaps and gutters. Opened photographs initially fit within 78% of the viewport height, retaining their full aspect ratio. A centered “expand” button below the photo switches to the available page width; “collapse” restores the fitted view. Expanded tall images can be scrolled. Detail image `sizes` follow the selected view and account for the page gutters. Browsers select the appropriate existing WebP variant without changing image quality settings.
